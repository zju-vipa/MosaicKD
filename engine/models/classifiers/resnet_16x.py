...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        self.conv1 = nn.Conv2d(3, num_filters[0], kernel_size=3, padding=1, stride=2,
                               bias=False)
    ...
    def forward(self, x, return_features=False, return_rep=False):
        ...
        x = self.bn1(x)
        x = self.relu(x)  # 32x32
        f1 = x = self.layer1(x)  # 32x32
        f2 = x = self.layer2(x)  # 16x16
        f3 = x = self.layer3(x)  # 8x8
        x = self.avgpool(x)
